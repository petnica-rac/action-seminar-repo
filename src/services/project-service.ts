  ...
  try {
    ...
    for (const project of projects) {
      try {
        ...
        const sourceStatusField = project.fields.nodes.find(
          (f) => f.name === 'Status'
        )
        if (
          sourceStatusField &&
          ...
          try {
            core.info('Updating Status field to match source project...')
            ...
            const targetStatusField = targetStatusData.node.fields.nodes.find(
              (f) => f.name === 'Status'
            )
            ...
            if (targetStatusField) {
              ...
              core.debug(
                `Deleting ${targetStatusField.options.length} existing Status options...`
              )
              for (const option of targetStatusField.options) {
                try {
                  ...
                  core.debug(`Deleted option: ${option.name}`)
                } catch (error) {
                  core.warning(
                    `Failed to delete Status option "${option.name}": ${error instanceof Error ? error.message : error}`
                  )
                }
              }
              ...
              core.debug(
                `Adding ${sourceStatusField.options.length} Status options from source...`
              )
              for (const option of sourceStatusField.options) {
                try {
                  ...
                  core.debug(`Created Status option: ${option.name}`)
                } catch (error) {
                  core.warning(
                    `Failed to create Status option "${option.name}": ${error instanceof Error ? error.message : error}`
                  )
                }
              }

              core.info('Status field updated successfully')
            } else {
              core.warning('Could not find Status field in target project')
            }
          } catch (error) {
            core.warning(
              `Failed to update Status field: ${error instanceof Error ? error.message : error}`
            )
          }
        }
